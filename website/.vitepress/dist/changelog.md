---
url: 'https://azrod.github.io/opencode-team-lead/changelog.md'
---
# Changelog
...
* The team-lead now prefers registered user-defined agents over invented personas — when a project defines domain-specific agents (e.g., `languages/typescript-pro`, `mcp/mcp-developer`, `web/react-specialist`), it selects them instead of defaulting to a `general` + invented persona name.
...
* The delegation prompt template has been removed — modern models structure delegations well without an explicit template.
* In-Flight Delegations tracking from the scratchpad template has been removed — the compaction hook already preserves the full scratchpad.
* The Self-Evaluation numbered checklist has been collapsed to prose.
* The `< 1-2 delegations away` interruption threshold has been removed from the scope-switching protocol.
* The "When NOT to Prune" subsection has been removed from the Context Management section.
* The "Max Retries" column has been removed from the error handling retry table.

## [0.7.0] - 2026-03-25

### Added

* Three specialized reviewer agents are now included: `requirements-reviewer`, `code-reviewer`, and `security-reviewer` — the review-manager spawns them automatically based on change type and risk level (size × risk axes).
...
* The scratchpad now tracks the active task in detail — if compaction hits mid-step, the team-lead can resume exactly where it left off instead of losing implementation details.
